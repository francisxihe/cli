...
export default defineComponent({
  ...
  setup(props, { emit }) {
    ...
    const onClickSubmitForm = async () => {
      try {
        await SystemManage.roleBindDeptList({
          roleId: props.data.id,
          deptIdList: checkedDeptIdList.value,
        });
        emit('confirm');
        onClose();
    ...
    watch(
      ...
      async visible => {
        if (visible === true) {
          const res = await SystemManage.getDeptListByRoleId(props.data.id);
          checkedDeptIdList.value = res.data.map(item => item.id);
        } else {
          onClose();
        }
      },
