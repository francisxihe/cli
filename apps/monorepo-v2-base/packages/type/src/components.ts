...
export type TableColumnConfig<R = unknown> = Partial<{
  ...
  renderHeader: { (col): any };
  sortable: boolean | 'custom';
  sortMethod: { (a, b): any };
  sortBy: string | any[] | { (row: R, index): any };
  sortOrders: 'ascending' | 'descending' | null[];
  resizable: boolean;
  formatter: { (row: R, column, cellValue, index: number): any };
  showOverflowTooltip: boolean;
  align: 'left' | 'center' | 'right';
  headerAlign: 'left' | 'center' | 'right';
  className: string;
  labelClassName: string;
  selectable: { (row: R, index): any };
  reserveSelection: boolean;
  filters: Array<{ text; value }>;
  filterPlacement: string;
  filterMultiple: boolean;
  filterMethod: { (value, row: R, column): any };
  filteredValue?: any[];
